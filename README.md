...
```
3. Чтобы получить идентификатор, страну и местоположение города, используя только часть его названия, предусмотрен метод *select_city_information(label_name, [country])*. Где параметр *[country]* является необязательным и задается выводом вышеупомянутой функции. Если он не задан, функции возвращают все города, которые соответствуют метке города. В примере указана информация обо всех городах в США, начинающаяся с метки *"san Ant"* (без учета регистра):
```
