...
**weather_api_class.py** was developed to provide a simple way to load weather data from the API at [https://openweathermap.org/](https://openweathermap.org/). It consists of two classes:

1. **CitiesInformation**: This class is designed to load the ID, country, and location of all cities included in the Weather API database. The list is provided in the file [city.list.json.gz](http://bulk.openweathermap.org/sample/city.list.json.gz), hosted on the server. The file is downloaded locally and unpacked into the `./city_list/` folder using one of the class's methods.

2. **OpenWeatherApi**: This class provides methods for accessing all queries available in the free account for each category.

Both classes must be imported into your Python script to use their methods, as described below.

---

## Class: `CitiesInformation()`

No parameters are required to instantiate this class. You can define the following variable:

`>>>cities_info = CitiesInformation()`

#### Методы:

1. Get the number of cities listed in the database:
```
print(cities_info)
>>>The total number of cities in the API: 209579
```
2. Some requests require the country to be specified. The label of each of them in the database is reported by calling the *country_list()* method without parameters. It returns an array with the country ID.
```
...
```
3. To get the ID, country, and location of a city using only part of its name, the *select_city_information(label_name, [country])* method is provided. Where the parameter *[country]* is optional and is set by the output of the above function. If it is not specified, the functions return all cities that match the city label. The example shows information about all cities in the United States, starting with the label *"sanAnt"* (case insensitive):
```
...
```
The example above shows the cities that correspond to the label *"sanAnt"*. The Pandas data frame is returned. Therefore, for cities starting with *"house"*, the following parameters can be used:
```
...
This class provides access to the database. Any request requires a valid API key provided by the server. Creating an instance of a class is done using the following line of code:
```
query_class = Openweathermap(api_key)
```
He has two fundamental methods that are quite closely related to each other with the same list of parameters, these are:

1. *query_preprocessing(query_type, query_name, parameters)*
2. *query_execution(query_type, query_name, parameters)*

The only difference is that the first one only provides the request that needs to be executed on the server, and the second one executes it. Then
#### Defining parameters:
1. **query_type**: according to the services provided by the API for a free account, the following group of requests can be used:

- Current Weather API, (query_type = 0): Access to current weather data for any location on Earth, including more than 200,000 cities, updated based on global models and data from more than 40,000 weather stations, [Ссылка](https://openweathermap.org/current)

- API forecast for 5 days / 3 hours, (query_type = 1): forecast for 5 days for each location or city. It includes weather data every 3 hours, [Ссылка](http://openweathermap.org/forecast5)

- UV index, (query_type = 2): Access to current/projected/historical UV data for more than 200,000 cities, [Ссылка](https://openweathermap.org/api/uvi)

2. **query_name**: defines the query to be executed based on the selected query type. The parameters of certain query labels are shown below:
...
The request name must be entered as a string between " ", and the parameters must be entered as a list [ ]

#### Examples:
1. The current weather in Houston based on the ID in the database (shown above):
```
...
2. San Antonio Weather Forecast:
```
